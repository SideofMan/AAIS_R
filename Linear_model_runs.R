...
my_columns <- 1:(2^4-1)
df$algorithm_run<-my_columns

`[` <- function(...) base::`[`(...,drop=FALSE)
tic()
for (c in columns_in_run){
  print(c)
  #------------START--------------#
  rm(list=ls()[! ls() %in% c("c","columns_in_run")])
  ...
  if (Target_function=='outerproduct'){
    ...
    columns=c
    linearX = as.matrix(unname(Hald[,columns]))
    n=dim(linearX)[1]
    linearX=cbind(ones(n,1),linearX)
    n=dim(linearX)[1]; k=dim(linearX)[2]
    Y = as.matrix(unname(Hald[,5]))
    
    linearX2 = linearX[,-1]
    BigSigma22 <<- solve(t(linearX2)%*%(diag(n) - 1/n*ones(n,n))%*%linearX2)
    linearX <<- linearX
  ...
  output=list(M=Proposal$M, ESS=ESS_final, MarLik=MarLik,Exact_MarLik=LinearModelML_exact(linearX,data))
  #------------END-------------#
  ...
  df[ROW,2]=output$M
  df[ROW,3]=output$ESS
  df[ROW,4]=output$MarLik
  df[ROW,5]=output$Exact_MarLik
  df[ROW,6]=df[ROW,4]/df[ROW,5]
